...
```{r  dpi= 150}
 ggplot(treemap1, aes(x = reorder(Agrupamento, qtde_servidores), y = qtde_servidores)) +
  geom_bar(stat = "identity", width = 0.5, fill = "#4682b4") +
  geom_text(
    aes(
      label = paste(
        label_number(scale = 1e0)(qtde_servidores), " - ",
        scales::percent(pct)
      )
    ),
    size = 3, hjust = -0.1, color = "gray30"
  ) +
  scale_y_continuous(
    breaks = seq(0, 800000, by = 300000),
    labels = label_number(scale_cut = cut_short_scale()),
    expand = expansion(mult = c(0.01, 0.01)),
    limits = c(0, 380000)  
  ) +
  scale_fill_manual("#4682b4") +
  guides(fill = guide_legend(reverse = TRUE)) +
  labs(title = "Servidores por Agrupamento", y = NULL, x = NULL) +
  coord_flip() +
